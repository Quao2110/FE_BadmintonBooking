...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  Future<void> _onGetAll(GetAllUsersEvent event, Emitter<UserState> emit) async {
    emit(const UserLoading());
    try { emit(UserListLoaded(await getAllUsers())); }
    catch (e) { emit(UserError(e.toString().replaceFirst('Exception: ', ''))); }
  }

  Future<void> _onGetById(GetUserByIdEvent event, Emitter<UserState> emit) async {
    emit(const UserLoading());
    try { emit(UserLoaded(await getUserById(event.id))); }
    catch (e) { emit(UserError(e.toString().replaceFirst('Exception: ', ''))); }
  }
  ...
  Future<void> _onUpdate(UpdateUserEvent event, Emitter<UserState> emit) async {
    ...
    try {
      ...
      emit(UserActionSuccess(message: 'Cập nhật thông tin thành công!', updatedUser: user));
    } catch (e) { emit(UserError(e.toString().replaceFirst('Exception: ', ''))); }
  }
